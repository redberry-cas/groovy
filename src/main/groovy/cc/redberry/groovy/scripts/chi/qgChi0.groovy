...
indicesInsertion.addInsertionRule(parse(''' M_aA^a'A'_b'B' '''), IndexType.LatinLower1)
indicesInsertion.addInsertionRule(parse(''' M_aA^a'A'_b'B' '''), IndexType.LatinUpper1)
...
assert (expandAll(Aj0 >> parse('A_{mn}[k1_m, k2_m] - A_{nm}[k2_m, k1_m]'))) == parse('0')

def temp = parse('P_m = k1_m + k2_m') >> (Aj0 >> parse('k1^m*A_{mn}[k1_m, k2_m]'))
...
def M = parse('M_pA = T_A*G_m*G^mn[k1_m-k3_m]*A_{np}[k1_m-k3_m, k2_m]')
M = G >> M
M = Aj0 >> M
...
println 'zzz'
def M2 = M >> parse('M2 = Tr[k3_m*G^m*M_pA*k1_n*G^n*M_q^A*P^pq[k2_m]]')
