...
indicesInsertion.addInsertionRule(parse(''' M_axyA^a'A'_b'B' '''), IndexType.LatinLower1)
indicesInsertion.addInsertionRule(parse(''' M_axyA^a'A'_b'B' '''), IndexType.LatinUpper1)
...
//Clebsch–Gordan coefficient
    ...
    def projector = parse('J_abcd = (J_ac*J_bd + J_ad*J_bc)/2 - J_ab*J_cd/3')
    projector = expand(J >> projector)
    addSymmetry('J_abcd', 1, 0, 2, 3)
    addSymmetry('J_abcd', 0, 1, 3, 2)
...
//effective vertex
    ...
    def A1 = differentiate(A[1], parse('q^b'))
    A1 = parse('q_a = 0') >> A1

    tr = [ContractIndices, parse('d^a_a = 4'), parse('P_m*P^m = 4*m**2')]
    A1 = tr >> expandAll(A1, tr as Transformation[])

    def Aj0 = expression(parse('A_{mnab}[k1_m, k2_m]'), A1)

    assert (expandAll(Aj0 >> parse('A_{mnab}[k1_m, k2_m] - A_{nmab}[k2_m, k1_m]'))) == parse('0')
    ...
    tr.add(parse('P_a*J^abcd = 0'))
    tr.add(parse('P_a*J^ab = 0'))


    def M = parse('M_puvA = T_A*G_m*G^mn[k1_m-k3_m]*A_{npuv}[k1_m-k3_m, k2_m]')
    M = G >> M
    M = Aj0 >> M
    ...
    println 'zzz'
    def M2 = M >> parse('M2 = Tr[k3_m*G^m*M_puvA*k1_n*G^n*M_qij^A*P^pq[k2_m]]*J^uvij')
